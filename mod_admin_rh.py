...
def render_rh(users, avals_db, obras_db, inst_acessos_db):
    cols_obrigatorias = {
        'Nome':'','Password':'','Tipo':'Técnico','Cargo':'Técnico','Email':'',
        'Telefone':'','Morada':'','Localidade':'','Concelho':'','Codigo_Postal':'',
        'Naturalidade':'','Nacionalidade':'Portugal','NIF':'','NISS':'','CC':'',
        'CC_Validade':'','DataNasc':'','Estado_Civil':'','Sexo':'','Dependentes':'0',
        'Profissao':'','Categoria_Profissional':'','Habilitacoes_Literarias':'',
        'Contacto_Emergencia':'','Nome_Emergencia':'','Grau_Parentesco':'',
        'Banco_IBAN':'','Observacoes':'','Tamanho_Camisola':'','Tamanho_Calca':'',
        'Tamanho_Botas':'','Local':'Não','PrecoHora':'15.0','PrecoHoraStatus':'',
        'PrecoHoraData':'','PIN':'0000','Foto':'','Campos_Bloqueados':'[]',
        'PDFs_Vistos':'[]','PDFs_Validados':'Não','PDFs_Validacao_Data':''
    }
    ...
    if avals_db.empty:
        avals_db = pd.DataFrame(columns=[
            ...
            'Nota_Trabalho_Eq','Nota_Proatividade','Nota_Comunicacao','Media','Comentarios'
        ])
    ...
    with tab_pessoal:
        ...
        with col1:
            ...
            with st.form("form_novo_colab"):
                ...
                with col_e: cod_postal = st.text_input("Código Postal", key="rh_cp")

                st.markdown("#### 🌍 Dados Pessoais")
                col_f, col_g = st.columns(2)
                with col_f:
                    naturalidade = st.text_input("Naturalidade", key="rh_naturalidade")
                    data_nasc    = st.date_input("Data de Nascimento", key="rh_datanasc",
                ...
                with col_g:
                    ...
                    estado_civil  = st.selectbox("Estado Civil",
                        ["Solteiro(a)","Casado(a)","Divorciado(a)","Viúvo(a)","União de Facto"], key="rh_ec")
                ...
                habilitacoes = st.selectbox("Habilitações Literárias",
                    ["4º Ano","6º Ano","9º Ano","12º Ano","Curso Técnico","Licenciatura","Mestrado","Doutoramento"],
                    key="rh_hab")
                ...
                with col_j: tam_camisola = st.selectbox("Camisola", ["XS","S","M","L","XL","XXL","XXXL"], key="rh_tam_cam")
                with col_k: tam_calca   = st.selectbox("Calça", ["XS (34/36)","S (38)","M (40/42)","L (42/44)","XL (46/48)","XXL (50/52)"], key="rh_tam_calc")
                with col_l: tam_botas   = st.selectbox("Botas", ["40","41","42","43","44","45","Outro"], key="rh_tam_bot")
                    ...
                    if nome and telefone and nif and password:
                        if nome in users['Nome'].values:
                            ...
                            new_user = pd.DataFrame([{
                                ...
                                "Tipo": tipo, "Cargo": cargo, "Email": email,
                                "Telefone": telefone, "Morada": morada,
                                "Localidade": localidade, "Concelho": concelho,
                                "Codigo_Postal": cod_postal, "Naturalidade": naturalidade,
                                "Nacionalidade": nacionalidade, "NIF": nif, "NISS": niss,
                                "CC": cc,
                                ...
                                "Tamanho_Calca": tam_calca, "Tamanho_Botas": tam_botas,
                            ...
                            log_audit(usuario=st.session_state.user, acao="CRIAR_COLABORADOR",
                                      tabela="usuarios.csv", registro_id=nome,
        ...
        with col2:
            ...
            if not users.empty:
                cols_vis = [c for c in ['Nome','Tipo','Cargo','Telefone','Email','NIF','Local','PrecoHora'] if c in users.columns]
                ...
                for idx, user in users.iterrows():
                    ...
                    # ✅ CORRIGIDO: removido key=f"exp_{idx}" (não suportado pelo Streamlit)
                    with st.expander(f"👤 {nome_user} — {user.get('Cargo','N/A')}"):
                        ...
                        if st.session_state.get('tipo') == 'Admin':
                            st.divider()
                            st.markdown("**🔐 Gestão de Password**")
                            col_p1, col_p2 = st.columns(2)
                            with col_p1:
                                st.info("⚠️ Por segurança, a password não é visível")
                            with col_p2:
                                nova_pwd = st.text_input("Nova password", type="password", key=f"new_pwd_{idx}")
                                if st.button("🔄 Resetar Password", key=f"btn_reset_{idx}"):
                                    if nova_pwd:
                                        users.loc[idx, 'Password'] = hp(nova_pwd)
                                        save_db(users, "usuarios.csv")
                        ...
                        with col_a:
                            if st.button("✏️ Editar", key=f"edit_{idx}"):
                                st.info("Abre mod_perfil para edição completa.")
                        with col_b:
        ...
        if not users.empty:
            ...
            if st.button("💾 Guardar Avaliação", key="btn_avalia"):
                ...
                nova_av = pd.DataFrame([{
                    ...
                    "Nota_Tecnica": nota_tec, "Nota_Pontualidade": nota_pont,
                    "Nota_Trabalho_Eq": nota_eq, "Nota_Proatividade": nota_proat,
                    "Nota_Comunicacao": nota_com,
                    "Media": round(media, 2), "Comentarios": comentarios
                }])
    ...
    with tab_pdfs:
        ...
        **Instruções:**
        ...
        6. **PDF novo adicionado → todos validam novamente**
        """)

        try:
            pdfs_db = load_db("pdfs_obrigatorios.csv", ["ID","Nome","Descricao","Data_Upload","Upload_Por","Ficheiro_b64"])
        except:
        ...
        if not pdfs_db.empty:
            ...
            for idx, pdf in pdfs_db.iterrows():
                with st.expander(f"📄 {pdf.get('Nome','PDF')} — {pdf.get('Data_Upload','N/A')}", expanded=True):
                    ...
                    if pdf.get('Ficheiro_b64'):
                        try:
                            ...
                            st.download_button("📥 Descarregar PDF", pdf_data,
                                f"{pdf.get('Nome','documento')}.pdf", "application/pdf",
                                key=f"dl_pdf_{idx}")
                        except:
        ...
        with st.form("form_upload_pdf"):
            ...
            if st.form_submit_button("📤 Upload de PDF", use_container_width=True, type="primary"):
                if nome_pdf and ficheiro_pdf:
                    pdf_b64 = base64.b64encode(ficheiro_pdf.read()).decode()
                    novo_pdf = pd.DataFrame([{
                        "ID": str(uuid.uuid4())[:8].upper(),
                        "Nome": nome_pdf, "Descricao": desc_pdf,
                    ...
                    # Resetar validação de todos
                    for idx_u, _ in users.iterrows():
                        users.loc[idx_u, 'PDFs_Validados']     = 'Não'
                        users.loc[idx_u, 'PDFs_Vistos']        = json.dumps([])
                        users.loc[idx_u, 'PDFs_Validacao_Data']= ''
                    save_db(users, "usuarios.csv")

                    log_audit(usuario=st.session_state.user, acao="UPLOAD_PDF_OBRIGATORIO",
                              tabela="pdfs_obrigatorios.csv", registro_id=novo_pdf['ID'].iloc[0],
                              detalhes=f"PDF: {nome_pdf}. Reset para {len(users)} colaboradores", ip="")
                    ...
                    for _, u in users.iterrows():
                        if u.get('Tipo','') != 'Admin':
                            criar_notificacao(
                                ...
                                mensagem=f"Novo documento obrigatório: {nome_pdf}. Valida no teu perfil.",
                                tipo="warning", acao_url="/tecnico"
                            )
                            notificados += 1

                    inv()
                    st.success(f"✅ PDF '{nome_pdf}' carregado!")
                    st.info(f"🔄 {len(users)} colaboradores devem validar. {notificados} notificados.")
                    st.rerun()
                else:
