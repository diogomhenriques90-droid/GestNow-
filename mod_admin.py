...
def render_admin(users, obras_db, frentes_db, registos_db, faturas_db,
    ...
    with tab4:
        ...
        with st.expander("➕ Nova Obra"):
            with st.form("no_"):
                ...
                with c1:
                    ...
                    tipo_obra_cfg = st.selectbox(
                        "🏭 Tipo de Obra",
                        ["Normal","Instrumentação","Civil","Manutenção","Montagem"],
                        help="Obras de Instrumentação activam o módulo de gestão de instrumentos.")
        ...
        # ── Gestão de Equipa de Instrumentação ───────────────────────
        if inst_acessos_db is None:
            import pandas as _pd_ia
            inst_acessos_db = _pd_ia.DataFrame(
                columns=["Obra","Utilizador","Cargo","DataAcesso","Ativo"])

        obras_inst = obras_db[
            obras_db.get('TipoObra', pd.Series(dtype=str)) == 'Instrumentação'
        ]['Obra'].tolist() if not obras_db.empty and 'TipoObra' in obras_db.columns else []

        if obras_inst:
            st.markdown("---")
            st.markdown("#### 🔧 Gestão de Equipa — Obras de Instrumentação")
            oi_sel = st.selectbox("Obra de Instrumentação",
                obras_inst, key="oi_obra_sel")

            equipa_obra = inst_acessos_db[
                inst_acessos_db['Obra'] == oi_sel
            ] if not inst_acessos_db.empty else pd.DataFrame()

            col_eq1, col_eq2 = st.columns([3,2])

            with col_eq1:
                st.markdown("**👥 Equipa actual:**")
                if equipa_obra.empty:
                    st.caption("Sem colaboradores adicionados.")
                else:
                    for _, eq_row in equipa_obra.iterrows():
                        ativo = str(eq_row.get('Ativo','Sim')) in ('Sim','sim','1','true','True')
                        ic_eq = "✅" if ativo else "🚫"
                        col_ea, col_eb = st.columns([4,1])
                        with col_ea:
                            st.markdown(
                                f"{ic_eq} **{eq_row.get('Utilizador','—')}** "
                                f"<small style='color:#7A8BA6'>({eq_row.get('Cargo','—')})"
                                f" — desde {eq_row.get('DataAcesso','—')}</small>",
                                unsafe_allow_html=True)
                        with col_eb:
                            btn_lbl = "🚫 Retirar" if ativo else "✅ Reativar"
                            if st.button(btn_lbl,
                                    key=f"eq_tog_{eq_row.get('Utilizador','')}_{oi_sel}",
                                    use_container_width=True):
                                ia_upd = inst_acessos_db.copy()
                                mask = (ia_upd['Obra']==oi_sel) &                                        (ia_upd['Utilizador']==eq_row.get('Utilizador',''))
                                ia_upd.loc[mask,'Ativo'] = 'Não' if ativo else 'Sim'
                                save_db(ia_upd, "inst_acessos.csv")
                                inv(); st.rerun()

            with col_eq2:
                st.markdown("**➕ Adicionar colaborador:**")
                users_lista = users['Nome'].tolist() if not users.empty else []
                equipa_actual = equipa_obra['Utilizador'].tolist()                     if not equipa_obra.empty else []
                users_fora = [u for u in users_lista if u not in equipa_actual]

                if not users_fora:
                    st.caption("Todos os colaboradores já adicionados.")
                else:
                    novo_user = st.selectbox("Colaborador",
                        users_fora, key="eq_novo_user")
                    novo_cargo_inst = st.selectbox("Papel na instrumentação",
                        ["Instrumentista","Técnico de Campo",
                    ...
                    if st.button("➕ Dar Acesso",
                        ...
                        novo_acesso = pd.DataFrame([{
                            "Obra": oi_sel,
                            "Utilizador": novo_user,
                            "Cargo": novo_cargo_inst,
                            "DataAcesso": datetime.now().strftime('%d/%m/%Y'),
                            "Ativo": "Sim",
                        }])
                        save_db(
                            pd.concat([inst_acessos_db, novo_acesso],
                                ignore_index=True),
                            "inst_acessos.csv")
                        inv()
                        st.success(f"✅ {novo_user} adicionado à equipa de {oi_sel}!")
                        st.rerun()
