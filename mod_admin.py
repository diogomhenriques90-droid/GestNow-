...
import pandas as pd
from core import load_all, inv, fh, save_db, tem_permissao, _PERM_COLS, _SUPER_ADMINS, THEME
        ...
        if config:
            ...
            if st.button("📧 Enviar Email de Teste",
                ...
                if email_teste:
                    ...
                    st.warning("Insere um email.")
        else:
            st.warning("SMTP não configurado.")
...
def _tab_permissoes():
    ...
    if not admins:
        st.warning("Sem utilizadores com tipo Admin para gerir.")
        return
...
def _render_folhas_ponto_fat(folhas_db, folhas_ocr_db, obras_db):
    ...
    # Folhas assinadas
    st.markdown("#### Folhas Assinadas pelo Chefe")
    ...
    if not folhas_ocr_db.empty and 'Obra' in folhas_ocr_db.columns:
        ...
        if not ocr_obra.empty:
            ...
            st.markdown("---")
            st.markdown("#### Ver Folha de Ponto (Imagem)")
