...
def _calcular_irc(resultado_fiscal: float,
    ...
    if resultado_fiscal <= 0:
        return {
            ...
            "reserva_mensal":    0,   # ← sempre presente
            "regime":            regime
        }
...
def _gerar_saft(faturas_cli, faturas_forn,
    ...
    if not faturas_cli.empty and 'Data_Emissao' in faturas_cli.columns:
        ...
        for _, row in fc_mes.iterrows():
            ...
            ET.SubElement(line,"Description").text = str(row.get('Descricao','Servicos'))[:60]
    ...
    ET.SubElement(si,"NumberOfEntries").text = str(n_fat)
...
def _grafico_iva_12m(faturas_cli, faturas_forn, ano):
    ...
    fig.add_trace(go.Bar(
        name='IVA Dedutivel (compras)',
        x=meses_pt, y=ded_vals,
        marker_color='#10B981',
        hovertemplate='%{x}<br>Dedutivel: \u20AC%{y:,.2f}<extra></extra>'
    ))
    ...
    fig.update_layout(
        title={'text':f'IVA — {ano}','font':{'color':'#F1F5F9'}},
        barmode='group',
        height=300,
        ...
        yaxis2={'overlaying':'y','side':'right','tickprefix':'\u20AC',
                'tickfont':{'color':'#3B82F6'},'showgrid':False},
...
def _grafico_irc_estimativa(irc: dict):
    fig = go.Figure(go.Waterfall(
        ...
        x=["Resultado Fiscal","Taxa 17% (ate 50k)","Taxa 21% (resto)","Derrama (1.5%)","TOTAL"],
    ...
    fig.update_layout(
        title={'text':'Decomposicao IRC Estimado','font':{'color':'#F1F5F9'}},
        height=280,
        paper_bgcolor='rgba(0,0,0,0)',
        plot_bgcolor='rgba(30,41,59,0.5)',
        font={'color':'#F1F5F9'},
...
def _grafico_calendario_fiscal_heatmap(obrigacoes: list, cumpridas: set):
    ...
    fig = go.Figure(go.Heatmap(
        ...
        showscale=False,
        hovertemplate='%{y} — %{x}: %{z} obrigacao(oes)<extra></extra>'
    ))
    fig.update_layout(
        title={'text':'Mapa de Obrigacoes Fiscais','font':{'color':'#F1F5F9'}},
...
def _grafico_retencoes_ano(fat_forn, ano):
    ...
    for m in range(1, 13):
        ...
        if not fat_forn.empty and 'Data' in fat_forn.columns:
            ff = fat_forn.copy()
            ff['Data_d'] = pd.to_datetime(ff['Data'], dayfirst=True, errors='coerce')
            ff['Ret_Num'] = pd.to_numeric(ff.get('Retencao_Val',0), errors='coerce').fillna(0)
            mask = (
    ...
    fig = go.Figure(go.Bar(
        x=meses_pt, y=vals,
        marker_color=['#EF4444' if v > 0 else '#334155' for v in vals],
        text=[f"\u20AC{v:,.0f}" if v > 0 else "" for v in vals],
        textposition='outside',
        textfont={'color':'#F1F5F9','size':9},
        hovertemplate='%{x}<br>Retencoes: \u20AC%{y:,.2f}<extra></extra>'
    ))
    fig.update_layout(
        title={'text':f'Retencoes na Fonte — {ano}','font':{'color':'#F1F5F9'}},
        height=240,
        paper_bgcolor='rgba(0,0,0,0)',
        plot_bgcolor='rgba(30,41,59,0.5)',
        font={'color':'#F1F5F9'},
...
def _gerar_pdf_mapa_fiscal(iva_anual: list,
    ...
    normal_s = ParagraphStyle('normal', parent=styles['Normal'],
    ...
    meses_pt = ["Jan","Fev","Mar","Abr","Mai","Jun",
                "Jul","Ago","Set","Out","Nov","Dez"]
    iva_header = ["Mes","IVA Liquidado","IVA Dedutivel","IVA a Pagar","IVA a Recuperar"]
    iva_rows   = [iva_header]
    ...
    # FIX: usar .get() para reserva_mensal — pode estar ausente
    reserva_mensal_val = irc.get('reserva_mensal', 0)
    irc_rows = [
        ...
        ["Reserva Mensal Rec.",f"\u20AC{reserva_mensal_val:,.2f}"],
    ]
    ...
    story.append(Spacer(1, 0.3*cm))
    story.append(Paragraph(
        f"Retencoes na Fonte (total anual): \u20AC{retencoes_total:,.2f}",
        normal_s
    ))
...
def render_fat_fiscal(obras_db, registos_db,
    ...
    try:
        ...
        empresa = {"nome":"Correia Placido e Sousa, Lda.","nif":"517182718"}
    ...
    iva_anual_pagar = sum(r['iva_pagar'] for r in iva_anual)
    iva_mes_pagar   = iva_mes_atual['iva_pagar']

    fat_anual_est = _num(faturas_cli, 'Total')
    custo_sal_est = _num(rh_db,'Salario_Base') * 12 * 1.4 if not rh_db.empty else 0
    resultado_est = max(0, fat_anual_est - custo_sal_est)
    irc_est = _calcular_irc(resultado_est, ano_atual, "pme")

    c1,c2,c3,c4,c5 = st.columns(5)
    with c1: st.metric("IVA Mes Atual",   f"\u20AC{iva_mes_pagar:,.2f}")
    with c2: st.metric("IVA Anual Est.",  f"\u20AC{iva_anual_pagar:,.2f}")
    with c3: st.metric("IRC Estimado",    f"\u20AC{irc_est['total_a_pagar']:,.2f}")
    with c4: st.metric("Retencoes Fonte", f"\u20AC{ret_total:,.2f}")
    # FIX: usar .get() — reserva_mensal ausente quando resultado <= 0
    with c5: st.metric("Reserva/Mes IRC", f"\u20AC{irc_est.get('reserva_mensal', 0):,.2f}")
    st.divider()

    (t_iva, t_irc, t_ret,
     t_saft, t_cal, t_ss) = st.tabs([
        "IVA", "IRC", "Retencoes Fonte",
        "SAF-T", "Calendario Fiscal", "Seguranca Social",
    ])
    ...
    # ════════════════════════════════════════════════════════════
    with t_iva:
        st.markdown("### IVA — Mapa Anual")

        col_iva1, col_iva2 = st.columns(2)
        with col_iva1:
            ano_iva = st.number_input("Ano", min_value=2020,
                                       value=ano_atual, key="iva_ano")
        with col_iva2:
            regime_iva = st.selectbox("Regime", ["Mensal","Trimestral"],
                                       key="iva_regime")

        if ano_iva != ano_atual:
            iva_anual_calc = [
                _calcular_iva_mes(faturas_cli, faturas_forn, m, ano_iva)
                for m in range(1, 13)
            ]
        else:
        ...
        # FIX: key único
        st.plotly_chart(
            _grafico_iva_12m(faturas_cli, faturas_forn, ano_iva),
            use_container_width=True, key="iva_12m"
        )
        ...
        for i, r in enumerate(iva_anual_calc):
            ...
            rows_iva.append({
                ...
                "Dedutivel": f"\u20AC{r['iva_dedutivel']:,.2f}",
                "Saldo":     saldo_str,
            })
        ...
        with c2: st.metric("Total Dedutivel",  f"\u20AC{tot_ded_a:,.2f}")
        with c3: st.metric("Total a Pagar",    f"\u20AC{tot_pag_a:,.2f}")
        with c4: st.metric("Total a Recuperar",f"\u20AC{tot_rec_a:,.2f}")

        st.markdown("---")
        st.markdown(f"#### Detalhe — {meses_pt[mes_atual-1]} {ano_atual}")
        ...
        with col_ex1:
            csv_iva = df_iva.to_csv(index=False, encoding='utf-8-sig')
            st.download_button(
        ...
        with col_ex2:
            if st.button("Gerar Mapa Fiscal PDF", key="btn_pdf_fiscal",
    ...
    # ════════════════════════════════════════════════════════════
    with t_irc:
        st.markdown("### IRC — Imposto sobre Rendimento Coletivo")
        st.info("Estimativa de IRC baseada nos dados disponíveis. Taxa PME: 17% ate 50.000, 21% acima.")

        col_irc1, col_irc2 = st.columns(2)
        with col_irc1:
            irc_resultado = st.number_input(
                "Resultado Fiscal Estimado (euros)",
                min_value=-999999.0, value=max(0.0, resultado_est),
                step=5000.0, key="irc_resultado"
            )
            irc_regime = st.selectbox(
                "Regime IRC", ["pme","geral"], key="irc_regime",
                format_func=lambda x:
                    "PME — Taxa Reduzida 17%/21%" if x == "pme"
                    else "Geral — Taxa 21%"
            )
            irc_ano_calc = st.number_input(
                "Ano", min_value=2020, value=ano_atual, key="irc_ano"
            )

        irc_calc = _calcular_irc(irc_resultado, irc_ano_calc, irc_regime)
        ...
        if irc_resultado > 0:
            ...
            st.plotly_chart(
                ...
                use_container_width=True, key="irc_waterfall"
            )

        st.markdown("#### Pagamentos por Conta")
        st.info("Os pagamentos por conta sao 75% do IRC do ano anterior, pagos em julho e setembro.")
    ...
    # ════════════════════════════════════════════════════════════
    with t_ret:
        st.markdown("### Retencoes na Fonte")
        ...
        st.plotly_chart(
            ...
            use_container_width=True, key="ret_ano_fiscal"
        )
        ...
        for m in range(1, 13):
            ...
            prazo = f"20/{m+1:02d}/{ano_ret}" if m < 12 else f"20/01/{ano_ret+1}"
            rows_ret.append({
                "Mes":          meses_pt[m-1],
                "Fornecedores": n_forn,
                "Total Retido": f"\u20AC{val_m:,.2f}",
                "Prazo AT":     prazo,
                "Estado":       "OK" if val_m == 0 else "Pendente"
            })

        df_ret = pd.DataFrame(rows_ret)
        st.dataframe(df_ret, use_container_width=True, hide_index=True)
        st.metric("Total Retencoes Anuais", f"\u20AC{ret_total:,.2f}")

        st.markdown("---")
        st.markdown("#### Gerar Guia de Retencoes")

        col_gr1, col_gr2 = st.columns(2)
        with col_gr1:
            mes_guia = st.selectbox("Mes", meses_pt,
                                     index=mes_atual-1, key="ret_mes_guia")
        with col_gr2:
            if st.button("Gerar Guia PDF", key="btn_guia_ret_fiscal",
                         type="primary", use_container_width=True):
                mes_num_g = meses_pt.index(mes_guia) + 1
                retencoes_mes_g = []
                if not faturas_forn.empty and 'Data' in faturas_forn.columns:
                    ff_g = faturas_forn.copy()
                    ff_g['Data_d'] = pd.to_datetime(ff_g['Data'], dayfirst=True, errors='coerce')
                ...
                if retencoes_mes_g:
                    try:
                        from mod_fat_fornecedores import _gerar_guia_retencao
                        pdf_gr = _gerar_guia_retencao(
                            mes_num_g, ano_ret, retencoes_mes_g, empresa
                        )
                    ...
                    st.info(f"Sem retencoes em {mes_guia} {ano_ret}.")

        if st.session_state.get('guia_ret_f'):
            st.download_button(
                "Descarregar Guia Retencoes",
                data=st.session_state['guia_ret_f'],
                file_name=st.session_state.get('guia_ret_f_nome','guia.pdf'),
                mime="application/pdf",
                key="dl_guia_ret_f"
            )
    ...
    with t_saft:
        ...
        st.info("Obrigatorio mensalmente ate ao dia 5 do mes seguinte.")

        col_s1, col_s2 = st.columns(2)
        with col_s1:
            mes_saft = st.selectbox("Mes", meses_pt, index=mes_atual-1, key="saft_mes")
        with col_s2:
            ano_saft = st.number_input("Ano", min_value=2020,
        ...
        if not faturas_cli.empty and 'Data_Emissao' in faturas_cli.columns:
            ...
            fc_s['Data_d'] = pd.to_datetime(fc_s['Data_Emissao'], dayfirst=True, errors='coerce')
            fc_mes_s = fc_s[
                (fc_s['Data_d'].dt.month == mes_num_s) &
                (fc_s['Data_d'].dt.year  == ano_saft)
            ]
            n_fat_s = len(fc_mes_s)
            tot_s   = pd.to_numeric(fc_mes_s.get('Total',0), errors='coerce').fillna(0).sum()
        else:
            n_fat_s, tot_s = 0, 0.0

        col_sk1, col_sk2, col_sk3 = st.columns(3)
        with col_sk1: st.metric("Faturas no Mes", n_fat_s)
        with col_sk2: st.metric("Total Faturado", f"\u20AC{tot_s:,.2f}")
        with col_sk3:
        ...
        with col_saft1:
            if st.button("Gerar SAF-T XML", key="btn_saft",
                         type="primary", use_container_width=True):
        ...
        with col_saft2:
            if st.session_state.get('saft_xml'):
                st.download_button(
                    ...
                    file_name=st.session_state.get('saft_nome','SAFT.xml'),
        ...
        if st.session_state.get('saft_xml'):
            with st.expander("Preview SAF-T XML"):
                st.code(
        ...
        checklist_saft = [
            ...
            ("Todas as faturas do mes incluidas",      n_fat_s > 0),
            ("Tipos de fatura corretos (FT/FS/FR/NC)", True),
            ("Datas no formato YYYY-MM-DD",            True),
            ("NIF clientes preenchido",
             not faturas_cli.empty and 'NIF_Cliente' in faturas_cli.columns),
    ...
    with t_cal:
        ...
        ano_cal = st.number_input("Ano", min_value=2020,
        ...
        with col_cf1:
            tipo_filt_cal = st.selectbox(
                "Tipo", ["Todos"] + list({o['tipo'] for o in obrigacoes}),
                key="cal_tipo"
            )
        with col_cf2:
            urg_filt_cal = st.selectbox(
                "Urgencia", ["Todas","critica","alta","media","baixa"],
                key="cal_urg"
            )
        ...
        # FIX: key único
        st.plotly_chart(
            _grafico_calendario_fiscal_heatmap(obrigacoes, set()),
            use_container_width=True, key="cal_heatmap"
        )

        st.markdown("#### Proximas Obrigacoes")
        proximas = []
        ...
        if not proximas:
            st.success("Sem obrigacoes nos proximos 60 dias.")
        else:
        ...
        st.markdown("---")
        with st.expander("Ver todas as obrigacoes do ano"):
            all_rows = []
            for ob in sorted(
                obrig_filtradas,
                ...
                all_rows.append({
                    ...
                    "Urgencia":  ob['urgencia'].upper()
                })
            df_cal = pd.DataFrame(all_rows)
            st.dataframe(df_cal, use_container_width=True, hide_index=True)
            csv_cal = df_cal.to_csv(index=False, encoding='utf-8-sig')
            st.download_button(
                "Exportar Calendario",
    ...
    # ════════════════════════════════════════════════════════════
    with t_ss:
        st.markdown("### Seguranca Social")
        st.info("DRI ate dia 10 do mes seguinte. TSU empresa: 23.75% | TSU trabalhador: 11%.")

        ano_ss = st.number_input("Ano", min_value=2020, value=ano_atual, key="ss_ano")
        mes_ss = st.selectbox("Mes", meses_pt, index=mes_atual-1, key="ss_mes")
        mes_num_ss = meses_pt.index(mes_ss) + 1

        if rh_db.empty:
            st.info("Sem fichas RH. Adiciona colaboradores no tab RH Financeiro.")
        else:
            TSU_EMP  = 23.75
            TSU_TRAB = 11.00
            ...
            c1,c2,c3,c4 = st.columns(4)
            with c1: st.metric("Massa Salarial", f"\u20AC{tot_sal:,.2f}")
            with c2: st.metric("TSU Empresa",    f"\u20AC{tot_tsu_e:,.2f}")
            with c3: st.metric("TSU Trabalhador",f"\u20AC{tot_tsu_t:,.2f}")
            with c4: st.metric("TOTAL SS",       f"\u20AC{tot_ss:,.2f}")
            ...
            csv_ss = df_ss.to_csv(index=False, encoding='utf-8-sig')
            col_ss1, col_ss2 = st.columns(2)
            with col_ss1:
                st.download_button(
                    f"DRI {mes_ss} {ano_ss} CSV",
                    data=csv_ss.encode('utf-8-sig'),
                    file_name=f"DRI_{mes_num_ss:02d}_{ano_ss}.csv",
                    mime="text/csv",
                    key="dl_dri_ss"
                )
            with col_ss2:
                st.info("Para submissao oficial acede a: app.seg-social.pt/ptss")

            st.markdown("---")
            st.markdown("#### Projecao Anual SS")
            total_anual_ss = tot_ss * 12
            st.metric("Total SS Anual Estimado", f"\u20AC{total_anual_ss:,.2f}",
                      delta=f"\u20AC{tot_ss:,.2f}/mes")

            meses_proj = [meses_pt[m-1] for m in range(1, 13)]
            ...
            fig_ss.update_layout(
                title={'text':f'Contribuicoes SS {ano_ss}','font':{'color':'#F1F5F9'}},
                height=240,
                paper_bgcolor='rgba(0,0,0,0)',
                plot_bgcolor='rgba(30,41,59,0.5)',
                font={'color':'#F1F5F9'},
